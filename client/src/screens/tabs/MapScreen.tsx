...
import { Map as MapIcon, Navigation, Phone, LocateFixed, RefreshCw, Search, ChevronRight, Car, Footprints, Bike, Clock, MapPin, AlertCircle, Activity, Trash2, Mic, Activity as ActivityIcon, GraduationCap, Accessibility, MessageSquare, Smile } from "@tamagui/lucide-icons";
...
const SpeechTherapyMaps: React.FC = () => {
  ...
  const mapHTML = useMemo(() => {
    ...
    return generateMapHTML(location.latitude, location.longitude);
  ...
  const searchCenters = useCallback(async (lat: number, lng: number) => {
    try {
      ...
      .sort((a, b) => b._score - a._score || a.distance - b.distance)
      .slice(0, MAX_RESULTS);
  ...
  const handleWebViewMessage = (e: any) => {
    try {
      ...
      if (data.type === 'CENTER_SELECT') {
        const c = centers.find(x => Math.abs(x.latitude - data.center.latitude) < 0.0001 && Math.abs(x.longitude - data.center.longitude) < 0.0001);
            ...
            <Card bg="white" br={24} p="$4" elevation={10} shadowColor="#000" bw={1} bc={COLORS.sandMid}>
              <XStack gap="$3" ai="center" mb="$3">
                <Circle size={44} bg={COLORS.royalBlue + "0C"} bw={1} bc={COLORS.royalBlue + "15"} jc="center" ai="center">
                  <FacilityIconComponent type={selectedCenter.type} size={20} color={COLORS.royalBlue} />
                </Circle>
                <YStack f={1}>
                  ...
                  <XStack jc="center" gap="$2">
                    {['driving', 'walking', 'bicycling'].map(m => (
                      <Button 
                        ...
                        bw={1} bc={travelMode === m ? COLORS.royalBlue : COLORS.sandMid} 
                        icon={
                        ...
                        onPress={() => { setTravelMode(m as any); getRouteInfo(selectedCenter, m); }} 
              ...
              <XStack gap="$3">
                <Button f={1} bg={COLORS.royalBlue} color="white" br={14} icon={<Navigation size={18} color="white" />} onPress={() => openDirections(selectedCenter)}>Directions</Button>
      ...
      <YStack bg="white" borderTopLeftRadius={28} borderTopRightRadius={28} p="$4" pb={Platform.OS === 'android' ? 20 : 35} elevation={8} shadowColor={COLORS.deepNavy} shadowOpacity={0.08} bw={1} bc={COLORS.sandMid}>
        ...
        {isListExpanded && (
          <YStack>
            ...
            <XStack mt="$4" gap="$3">
              <Button f={1} bg={COLORS.mediumBlue} color="white" br={16} icon={<Search size={16} color="white" />} onPress={openGoogleMaps}>Find More on Google</Button>
